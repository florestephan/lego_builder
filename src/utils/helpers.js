...
export function createGeometry({
    ...
    for (let i = 0; i < dimensions.x; i++) {
        for (let j = 0; j < dimensions.z; j++) {
            const cylinder = new CylinderGeometry(knobDim, knobDim, knobDim, 10);
            const cylinder2 = new CylinderGeometry(knobDim, knobDim, knobDim, 10);
            const cylinder3 = new CylinderGeometry(knobDim, knobDim, knobDim, 10);
            const cylinder4 = new CylinderGeometry(knobDim, knobDim, knobDim, 10);
            const x = base * i - ((dimensions.x - 1) * base) / 2;
            const y = base / 1.5;
            const z = base * j - ((dimensions.z - 1) * base) / 2;
            cylinder.translate(6, 8, 6); // placement des cylindres
            cylinder2.translate(-6, 8, -6);
            cylinder3.translate(6, 8, -6);
            cylinder4.translate(-6, 8, 6);
            geometries.push(cylinder);
            geometries.push(cylinder2);
            geometries.push(cylinder3);
            geometries.push(cylinder4);
        }
    }
